...
bool CKey::SignCompact(const uint256 &hash, std::vector<unsigned char>& vchSig) const {
	...
	int ret = 1;//tql_debug secp256k1_ecdsa_sign_recoverable(secp256k1_context_sign, &sig, hash.begin(), begin(), secp256k1_nonce_function_rfc6979, nullptr);
    assert(ret);
    //tql_debug secp256k1_ecdsa_recoverable_signature_serialize_compact(secp256k1_context_sign, &vchSig[1], &rec, &sig);
    assert(ret);
    assert(rec != -1);
